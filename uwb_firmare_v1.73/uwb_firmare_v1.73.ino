...
IPAddress staticIP(192, 168, 161, 254); // Static IP address
IPAddress gateway(192, 168, 161, 31);     // Gateway IP address
...
float anchor_matrix[N_ANCHORS][3] = { //list of anchor coordinates
  ...
  float anchor_matrix1[N_ANCHORS][3] = { //list of anchor coordinates
  {0.0, -1.22, 0.82},
  {2.92, 0.0, 0.84},
  {2.93, 6.80, 0.73},
  {2.78, 0.0, 0.05}
 ...
 float x1 = anchor_matrix1[0][0], Y11 = anchor_matrix1[0][1], z1 = anchor_matrix1[0][2];
  float x2 = anchor_matrix1[1][0], y2 = anchor_matrix1[1][1], z2 =  anchor_matrix1[1][2];
  float x3 = anchor_matrix1[2][0], y3 = anchor_matrix1[2][1], z3 =  anchor_matrix1[2][2];
  float x4 = anchor_matrix1[3][0], y4 = anchor_matrix1[3][1], z4 =  anchor_matrix1[3][2];
...
void setup()
{
  ...
  float x4 = anchor_matrix1[3][0], y4 = anchor_matrix1[3][1], z4 =  anchor_matrix1[3][2];
  #ifdef DEBUG_EEPROM
        Serial.print("x1 y2 z4");  //result current tag pos
        Serial.print(x1);
        Serial.write(',');
        Serial.print(y2);
        Serial.write(',');
...
void newRange()
{
  ...
  if ( (detected == N_ANCHORS)) { //four recent measurements
    ...
    //    GPSpreviousMillis = GPScurrentMillis;  // Save the last time you sent data
    //   // mav.FakeGPS(rotateLatitude, rotateLongitude, Avg_tag_position[2], time_week, time_week_ms);
    // }
      ...
      Serial.write(',');
      Serial.print(Avg_tag_position[2]);
    ...
    for(k1=0; k1<nk; k1++) {//average several position estimates
      ...
      for (int i = 0; i < N_ANCHORS; i++) {
        ...
        //  double dz = current_tag_position[2] - anchor_matrix[i][2];
         double dx = anchor_matrix1[i][0] - current_tag_position[0];
         double dy = anchor_matrix1[i][1] - current_tag_position[1];
         double dz = anchor_matrix1[i][2] - current_tag_position[2];
