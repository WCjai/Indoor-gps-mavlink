...
void handleRoot() {
  ...
  page += "  .input-label { width: 50px; margin-right: 10px; }";
  page += "  .input-field { padding: 8px; border: 1px solid #ccc; border-radius: 5px; flex: 1; }";
  page += "</style>";
  page += "</head>";
  page += "<body>";
  ...
  page += "<h1 style='color: #333; flex: 1;'>RTLS Configuration</h1>";
  page += "<img src='https://img1.wsimg.com/isteam/ip/f1c33a55-e3a9-4a04-aaf1-f1a2346a7b37/indronovation%20logo.png/:/rs=h:200,cg:true,m/qt=q:95' alt='RTLS Logo' style='width: 200px; margin-left: 20px;'>";
  page += "</div>";
  ...
  for (int i = 0; i < N_ANCHORS; i++) {
    ...
    page += "<div class='input-container'>";
    page += "<input type='text' name='anchor_x_" + String(i) + "' value='" + String(anchor_matrix[i][0]) + "' class='input-field' style='margin-right: 10px;'>";
    page += "<input type='text' name='anchor_y_" + String(i) + "' value='" + String(anchor_matrix[i][1]) + "' class='input-field' style='margin-right: 10px;'>";
    page += "<input type='text' name='anchor_z_" + String(i) + "' value='" + String(anchor_matrix[i][2]) + "' class='input-field'>";
    page += "</div>";
  }

  // Toggle button for tag position
  page += "<button onclick='toggleTagPosition()'>Toggle Tag Position</button><br>";

  // Submit button
  page += "<input type='submit' value='Update' style='background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer;'>";

  // 3D plot
  page += "<div id='graph'></div>";

  // JavaScript for plotting and toggle
  page += "<script>";
  page += "var showTagPosition = true;";
  page += "function updatePlot() {";
  page += "  var anchorPositions = [";
  for (int i = 0; i < N_ANCHORS; i++) {
    page += "{ x: " + String(anchor_matrix[i][0]) + ", y: " + String(anchor_matrix[i][1]) + ", z: " + String(anchor_matrix[i][2]) + " }";
    if (i < N_ANCHORS - 1) page += ", ";
  }
  page += "];";
  page += "if (showTagPosition) {";
  page += "  var tagPosition = { x: " + String(Avg_tag_position[0]) + ", y: " + String(Avg_tag_position[1]) + ", z: " + String(Avg_tag_position[2]) + " };";
  page += "  anchorPositions.push(tagPosition);";
  page += "}";
  ...
  page += "}];";
  page += "var layout = {";
  page += "  title: 'Anchor and Tag Positions',";
  page += "  scene: {";
  page += "    xaxis: { title: 'X' },";
  page += "    yaxis: { title: 'Y' },";
  page += "    zaxis: { title: 'Z' }";
  ...
  page += "}";
  page += "function toggleTagPosition() {";
  page += "  showTagPosition = !showTagPosition;";
  page += "  updatePlot();";
  page += "}";
  page += "updatePlot();"; // Initial plot setup
  page += "</script>";
