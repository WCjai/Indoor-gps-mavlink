...
void updatePlot() {
  String updateScript = "tagPosition = { x: " + String(Avg_tag_position[0]) + ", y: " + String(Avg_tag_position[1]) + ", z: " + String(Avg_tag_position[2]) + " };"; // Update tag position
  updateScript += "anchorPositions.pop();"; // Remove previous tag position
  updateScript += "anchorPositions.push(tagPosition);"; // Add updated tag position
  updateScript += "Plotly.redraw('graph');"; // Redraw plot
  server.send(200, "text/html", updateScript); // Send update script to client
}
void handleRoot() {
